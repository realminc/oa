// OaDsTinyShakespeare — TinyShakespeare text dataset (OaDataset subclass)
//
// Loads plain-text Shakespeare plays (single continuous text file).
// Each sample is a context window of N bytes + the next byte as label.
//
// Usage:
//   OaDsTinyShakespeare ds("/data/tiny_shakespeare/tiny_shakespeare.txt", 64, 128);
...
class OaDsTinyShakespeare : public OaDataset {
public:
	/// Load TinyShakespeare dataset from text file.
	/// @param InFilePath Path to tiny_shakespeare.txt
	...
	OaDsTinyShakespeare(const OaString& InFilePath, OaI32 InBatchSize = 64, OaI32 InContextLen = 128, bool InShuffle = true);
