...
//
// Fallible value-producing and mutating calls return OaStatus/OaResult.
...
// OaFilesystem — stateless host-filesystem operations.
//
// OaPath owns lexical path manipulation. OaPaths owns OA location discovery.
class OaFilesystem {
public:
	// ─── Existence & Info ─────────────────────────────────────────────────
	...
	// Unlike OaPath's lexical operations, Absolute queries process state.
	[[nodiscard]] static OaResult<OaPath> Absolute(const OaPath &InPath);
