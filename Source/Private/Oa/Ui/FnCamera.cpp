...
VlmMat4 GetViewMatrix(const OaCameraState& InState) noexcept {
	...
	VlmMat4 view = VlmMat4::Identity();
	view.M[0][0] = rot.M[0][0]; view.M[0][1] = rot.M[1][0]; view.M[0][2] = rot.M[2][0];
	view.M[1][0] = rot.M[0][1]; view.M[1][1] = rot.M[1][1]; view.M[1][2] = rot.M[2][1];
	view.M[2][0] = rot.M[0][2]; view.M[2][1] = rot.M[1][2]; view.M[2][2] = rot.M[2][2];

	// Translation (inverse of camera position, rotated)
	VlmVec3 t = {
		-(view.M[0][0] * InState.Position.X + view.M[0][1] * InState.Position.Y + view.M[0][2] * InState.Position.Z),
		-(view.M[1][0] * InState.Position.X + view.M[1][1] * InState.Position.Y + view.M[1][2] * InState.Position.Z),
		-(view.M[2][0] * InState.Position.X + view.M[2][1] * InState.Position.Y + view.M[2][2] * InState.Position.Z)
	};
	view.M[3][0] = t.X;
	view.M[3][1] = t.Y;
	view.M[3][2] = t.Z;
