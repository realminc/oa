...
static OaEngine* GEngine = nullptr;
...
// wrappers, so their eventual deleters never need to dereference OaEngine.
class OaEngine::OaBufferLeaseRegistry {
public:
	void Attach(OaEngine& InOwner) {
		std::lock_guard<std::mutex> lock(Mutex_);
		Owner_ = &InOwner;
	}

	[[nodiscard]] bool Register(OaEngine& InOwner, OaVkBuffer* InBuffer) {
	...
	void Release(OaVkBuffer* InBuffer) {
		if (not InBuffer) return;
		{
			std::lock_guard<std::mutex> lock(Mutex_);
			for (OaUsize i = 0; i < Entries_.Size(); ++i) {
	...
	void Drain(OaEngine& InOwner) {
		...
		for (OaUsize i = Entries_.Size(); i > 0; --i) {
...
private:
	static void ReleaseStorage_(
		OaEngine& InOwner, OaVkBuffer& InBuffer, OaBool InClosing)
	{
		...
		if (InBuffer.NodeIndex == 0) InOwner.FreeBuffer(InBuffer);
		else InOwner.FreeBufferOnNode(InBuffer);
		InBuffer = {};
	}

	OaEngine* Owner_ = nullptr;
	OaVec<OaVkBuffer*> Entries_;
	std::mutex Mutex_;
};

OaEngine::OaEngine()
	: BufferLeaseRegistry_(OaMakeSharedPtr<OaBufferLeaseRegistry>())
{
	BufferLeaseRegistry_->Attach(*this);
}
...
OaEngine* OaEngine::GetGlobal() { return GEngine; }

OaContext& OaEngine::GetContext() const noexcept {
	assert(Context_ && "OaEngine::GetContext: engine is not initialized");
	return *Context_;
}
...
void OaEngine::LogSelectedDevices() {
...
OaSharedPtr<OaVkBuffer> OaEngine::AdoptBufferLease_(
	OaVkBuffer&& InBuffer,
	OaSharedPtr<OaVkBuffer> InBacking)
{
	auto* buffer = new OaVkBuffer(std::move(InBuffer));
	auto registry = BufferLeaseRegistry_;
	if (not registry or not registry->Register(*this, buffer)) {
		if (buffer->NodeIndex == 0) FreeBuffer(*buffer);
		else FreeBufferOnNode(*buffer);
		delete buffer;
		return {};
	}
	return OaSharedPtr<OaVkBuffer>(buffer,
		[registry = std::move(registry), backing = std::move(InBacking)](
			OaVkBuffer* InPtr) mutable {
...
// OaEngine is pinned (move/copy = delete in the header). It owns a
...
OaResult<OaUniquePtr<OaEngine>> OaEngine::Create(const OaEngineConfig& InConfig) {
	...
	auto engine = OaMakeUniquePtr<OaEngine>();
	OA_RETURN_IF_ERROR(engine->InitInPlace(InConfig));
	return engine;
}

OaStatus OaEngine::InitInPlace(const OaEngineConfig& InConfig) {
...
OaStatus OaEngine::InitInPlaceImpl(const OaEngineConfig& InConfig) {
	...
	const OaBool wantHeadlessGraphics =
		InConfig.PresentationMode == OaPresentationMode::Headless;
...
OaStatus OaEngine::Close() {
	...
	if (!Device.Device) {
		if (BufferLeaseRegistry_) BufferLeaseRegistry_->Drain(*this);
	...
	for (auto& stream : StreamPool_) {
		if (stream && stream->Submitted) retainError(stream->Synchronize(Device));
	}
	for (auto& stream : AsyncStreamPool_) {
		if (stream && stream->Submitted) retainError(stream->Synchronize(Device));
	}
	...
	retainError(CompleteRetiredExecutionPlans_());
	retainError(CompleteRetiredContextBatches_());
	retainError(CompleteRetiredUploadRings_());
	retainError(CompleteRetiredPresenters_());
...
void OaEngine::Destroy() {
	if (auto status = Close(); !status.IsOk()) {
		OA_LOG_ERROR(OaLogComponent::Core,
			"OaEngine::Destroy: shutdown failed: %s",
			status.ToString().c_str());
	}
}

void OaEngine::RetireBorrowedService_(
	void* InPayload,
	...
	OaRetiredServiceState::ReleaseFn InRelease)
{
	...
	RetiredBorrowedServices_.EmplaceBack(
		InPayload, InComplete, InRelease);
}

OaStatus OaEngine::CompleteRetiredBorrowedServices_() {
	OaVec<OaRetiredServiceState> retired;
	{
		OaScopedLock<OaMutex> lock(RetiredBorrowedServiceMutex_);
...
static void OaLoadSpvDirIntoPipelines(
	OaEngine& InRt, const OaString& InDir, OaU32* InOutLoaded, OaU32* InOutSkipped)
{
...
OaStatus OaEngine::EnsureAllEmbeddedLiboaPipelines() {
	OaU32 loaded = 0;
	OaU32 skipped = 0;
	OaU32 failed = 0;
...
OaComputePipeline& OaEngine::GetPipeline(OaStringView InName) {
	return Pipelines.GetPipeline(InName, DtypeSpecConstant());
}

OaComputePipeline& OaEngine::GetPipeline(OaKernelId InKernelId) {
...
void OaEngine::AddShaderSearchPath(const OaString& InPath) {
...
OaResult<OaVkBuffer> OaEngine::AllocBuffer(OaU64 InSize) {
	return AllocBuffer(InSize, OaMemoryPlacement::HostUpload);
}

OaResult<OaVkBuffer> OaEngine::AllocBuffer(
	OaU64 InSize, OaMemoryPlacement InPlacement)
{
	...
	if (placement == OaMemoryPlacement::HostReadback) {
		...
		if (result && RegisterBuffer(*result) == OA_BINDLESS_INVALID) {
...
OaStatus OaEngine::UploadBuffer(
	const OaVkBuffer& InDst, OaU64 InDstOffset, const void* InData, OaU64 InSize)
{
	...
	if (!UploadRing_ || UploadRing_->FrameCapacityBytes() < copySize) {
		if (UploadRing_) {
			OA_RETURN_IF_ERROR(UploadRing_->Close());
			UploadRing_.reset();
		}
...
OaStatus OaEngine::ReadbackBuffer(
	const OaVkBuffer& InSrc, OaU64 InSrcOffset, void* OutData, OaU64 InSize)
{
	...
	if (InSrc.Allocation == nullptr or InSrc.AliasIdentity != nullptr
		or InSrc.NodeIndex != 0U or InSrc.IsImported()
		or InSrc.AllocatorIdentity != Allocator.Allocator) {
		return OaStatus::InvalidArgument(
			"ReadbackBuffer: source must be a non-aliased primary-engine VMA allocation");
	}

	std::lock_guard<std::mutex> lock(ReadbackMutex_);
	if (not ReadbackStream_.CommandPool) {
	...
	if (InSrc.MappedPtr) {
		...
		if (status.IsOk()) {
			ReadbackStream_.RecordHostReadbackBarrier();
		...
		if (not Allocator.InvalidateHostBuffer(InSrc, InSrcOffset, InSize)) {
	...
	if (not ReadbackStaging_.Buffer or ReadbackStaging_.Capacity < copySize) {
	...
	if (status.IsOk()) {
		...
		ReadbackStream_.RecordTransferReadBarrier(
		...
		ReadbackStream_.RecordTransferWriteBarrier(
			ReadbackStaging_, 0U, copySize);
		status = ReadbackStream_.Submit(*this);
	}
	if (status.IsOk()) status = ReadbackStream_.Synchronize(Device);
	if (status.IsOk() and not Allocator.InvalidateHostBuffer(ReadbackStaging_, 0, copySize)) {
...
OaResult<OaVkBuffer> OaEngine::AllocBufferDevice(OaU64 InSize) {
...
OaResult<OaVkBuffer> OaEngine::AllocBufferBar(OaU64 InSize) {
...
void OaEngine::FreeBuffer(OaVkBuffer& InOutBuffer) {
...
OaU32 OaEngine::RegisterBuffer(OaVkBuffer& InOutBuffer) {
...
OaU32 OaEngine::RegisterBufferForOwnedNode(OaVkBuffer& InOutBuffer) {
...
void OaEngine::UpdateBufferDescriptor(const OaVkBuffer& InBuffer) {
	if (InBuffer.BindlessIndex == OA_BINDLESS_INVALID) return;
	if (InBuffer.NodeIndex == 0 || !Mesh_) {
		Bindless.Update(Device, InBuffer.BindlessIndex, InBuffer);
		return;
	}
	auto* node = GetNode(InBuffer.NodeIndex);
	if (!node) return;
	OaSpinlockGuard guard(*Mesh_->DeviceLoadLock);
	OaVkLoadDevice(static_cast<VkDevice>(node->Device.Device));
	node->Bindless.Update(node->Device, InBuffer.BindlessIndex, InBuffer);
	OaVkLoadDevice(static_cast<VkDevice>(Device.Device));
}

void OaEngine::DeregisterBuffer(OaVkBuffer& InOutBuffer) {
...
OaVkStream* OaEngine::AcquireStream() {
	CollectRetiredImageDispatches_();
	CollectRetiredExecutionPlans_();
	CollectRetiredContextBatches_();
	...
	OaVec<VkImageView>&& InImageViews)
{
	...
	OaRetiredImageDispatch retired;
	retired.Stream = InStream;
	retired.StorageImageSlots = OaStdMove(InStorageImageSlots);
	retired.SampledImageSlots = OaStdMove(InSampledImageSlots);
	retired.SamplerSlots = OaStdMove(InSamplerSlots);
	retired.ImageViews = OaStdMove(InImageViews);
	std::lock_guard<std::mutex> lock(RetiredImageDispatchMutex_);
	RetiredImageDispatches_.PushBack(OaStdMove(retired));
}

void OaEngine::CollectRetiredImageDispatches_()
{
	std::lock_guard<std::mutex> lock(RetiredImageDispatchMutex_);
	for (OaUsize i = RetiredImageDispatches_.Size(); i > 0; --i) {
		auto& retired = RetiredImageDispatches_[i - 1];
		if (retired.Stream == nullptr || !retired.Stream->IsComplete(Device)) {
			continue;
		}
		for (VkImageView view : retired.ImageViews) {
			if (view != VK_NULL_HANDLE) {
				vkDestroyImageView(static_cast<VkDevice>(Device.Device), view, nullptr);
			}
		}
		for (OaU32 idx : retired.StorageImageSlots) Bindless.DeregisterStorageImage(idx);
		for (OaU32 idx : retired.SampledImageSlots) Bindless.DeregisterSampledImage(idx);
		for (OaU32 idx : retired.SamplerSlots) Bindless.DeregisterSampler(idx);
		retired.Stream->Submitted = false;
		ReleaseStream(retired.Stream);
		RetiredImageDispatches_.Erase(RetiredImageDispatches_.Begin() + (i - 1));
	}
}

void OaEngine::RetireExecutionPlan(OaUniquePtr<OaComputeGraph>&& InGraph)
{
	if (not InGraph) return;
	OaScopedLock<OaMutex> lock(RetiredExecutionPlanMutex_);
	RetiredExecutionPlans_.PushBack(OaStdMove(InGraph));
}

void OaEngine::CollectRetiredExecutionPlans_()
{
	OaVec<OaUniquePtr<OaComputeGraph>> retired;
	{
		OaScopedLock<OaMutex> lock(RetiredExecutionPlanMutex_);
		retired = OaStdMove(RetiredExecutionPlans_);
	}

	OaVec<OaUniquePtr<OaComputeGraph>> pending;
	for (auto& graph : retired) {
		const auto completion = graph->LastCompletion(Device);
		if (completion.IsValid() and not completion.IsComplete()) {
			pending.PushBack(OaStdMove(graph));
			continue;
		}
	...
	if (not pending.Empty()) {
		...
		for (auto& graph : pending) {
			RetiredExecutionPlans_.PushBack(OaStdMove(graph));
		}
	}
}

OaStatus OaEngine::CompleteRetiredExecutionPlans_()
{
	...
	OaVec<OaUniquePtr<OaComputeGraph>> pending;
	for (auto& graph : retired) {
		const auto waitStatus = graph->WaitForPendingReplay(Device);
...
void OaEngine::RetireContextBatch(
	OaVkStream* InStream,
	const OaEvent& InCompletion,
	OaVec<OaUniquePtr<OaComputeGraph>>&& InGraphs)
{
	if (InStream == nullptr) return;
	OaRetiredContextBatch retired;
	retired.Stream = InStream;
	retired.Completion = InCompletion;
	retired.Graphs = OaStdMove(InGraphs);
	OaScopedLock<OaMutex> lock(RetiredContextBatchMutex_);
	RetiredContextBatches_.PushBack(OaStdMove(retired));
}

void OaEngine::CollectRetiredContextBatches_()
{
	OaVec<OaRetiredContextBatch> retired;
	{
		OaScopedLock<OaMutex> lock(RetiredContextBatchMutex_);
		retired = OaStdMove(RetiredContextBatches_);
	}

	OaVec<OaRetiredContextBatch> pending;
	for (auto& batch : retired) {
		// The stream owns the timeline and remains the authoritative retirement
		// proof even if a malformed completion token reached this fallback path.
		if (batch.Stream != nullptr and not batch.Stream->IsComplete(Device)) {
	...
	if (not pending.Empty()) {
		...
		for (auto& batch : pending) {
			RetiredContextBatches_.PushBack(OaStdMove(batch));
		}
	}
}

OaStatus OaEngine::CompleteRetiredContextBatches_()
{
	OaStatus result = OaStatus::Ok();
	OaVec<OaRetiredContextBatch> retired;
	{
	...
	for (auto& batch : retired) {
		const auto waitStatus = batch.Stream != nullptr
			? batch.Stream->Synchronize(Device)
			: OaStatus::Error(OaStatusCode::Internal,
...
void OaEngine::RetireUploadRing(OaUniquePtr<OaRetiredUploadRing>&& InRing)
{
	if (not InRing) return;
	OaScopedLock<OaMutex> lock(RetiredUploadRingMutex_);
	RetiredUploadRings_.PushBack(OaStdMove(InRing));
}

OaStatus OaEngine::CompleteRetiredUploadRings_()
{
	OaStatus result = OaStatus::Ok();
	OaVec<OaUniquePtr<OaRetiredUploadRing>> retired;
	{
		OaScopedLock<OaMutex> lock(RetiredUploadRingMutex_);
		retired = OaStdMove(RetiredUploadRings_);
	}

	OaVec<OaUniquePtr<OaRetiredUploadRing>> pending;
	for (auto& ring : retired) {
		OaBool complete = true;
		for (auto& frame : ring->Frames) {
			if (not frame) continue;
			const auto waitStatus = frame->Stream.Synchronize(Device);
			if (not waitStatus.IsOk()) {
				if (result.IsOk()) result = waitStatus;
				complete = false;
			}
		...
		if (not complete) {
			pending.PushBack(OaStdMove(ring));
			continue;
		}
		for (auto& frame : ring->Frames) {
			if (frame) frame->Stream.Destroy(Device);
		}
	...
	if (not pending.Empty()) {
		...
		for (auto& ring : pending) {
			RetiredUploadRings_.PushBack(OaStdMove(ring));
		}
	}
	return result;
}

void OaEngine::ReleaseStream(OaVkStream* InStream) {
	OaSpinlockGuard guard(StreamPoolLock_);
...
OaVkStream* OaEngine::AcquireAsyncStream() {
...
void OaEngine::ReleaseAsyncStream(OaVkStream* InStream) {
...
OaResult<OaEvent> OaEngine::CopyBufferAsync(
	...
	if (InSrc.NodeIndex != 0U or InDst.NodeIndex != 0U) {
		return OaStatus::InvalidArgument(
			"CopyBufferAsync: buffers must belong to the primary engine device");
	}
	if (InSrc.Allocation == nullptr or InDst.Allocation == nullptr
		or InSrc.AllocatorIdentity != Allocator.Allocator
		or InDst.AllocatorIdentity != Allocator.Allocator
		or InSrc.IsImported() or InDst.IsImported()
	...
	std::lock_guard<std::mutex> lock(TransferStreamMutex_);
	if (not TransferStream_.CommandPool) {
		...
		auto res = OaVkStream::Create(
			Device,
			Device.Queues.ComputeQueueFamily,
			Device.Queues.ComputeQueue
		);
		if (not res) {
	...
	if (TransferStream_.Submitted) {
		if (not TransferStream_.IsComplete(Device)) {
			return OaStatus::Error(OaStatusCode::FailedPrecondition,
				"CopyBufferAsync: previous transfer is still in flight; wait its OaEvent");
		}
		TransferStream_.Submitted = false;
	}
	OA_RETURN_IF_ERROR(TransferStream_.Begin(Device));
	TransferStream_.RecordTransferReadBarrier(InSrc, 0U, InSize);
	TransferStream_.RecordCopyBuffer(InSrc, InDst, InSize);
	TransferStream_.RecordTransferWriteBarrier(InDst, 0U, InSize);
	OA_RETURN_IF_ERROR(TransferStream_.Submit(*this));
	OaEvent completion = TransferStream_.Completion(Device);
...
OaU32 OaEngine::DeviceCount() const {
	return Mesh_ ? Mesh_->NodeCount() : 1;
}

OaDeviceNode* OaEngine::GetNode(OaU32 InIndex) {
	return Mesh_ ? Mesh_->GetNode(InIndex) : nullptr;
}

OaDeviceNode* OaEngine::GetPrimary() {
	return Mesh_ ? Mesh_->Primary : nullptr;
}

OaDeviceNode* OaEngine::GetAuxiliary() {
	return Mesh_ ? Mesh_->GetByRole(OaDeviceRole::Auxiliary) : nullptr;
}

bool OaEngine::IsMultiDevice() const {
	return Mesh_ && Mesh_->NodeCount() > 1;
}

OaVkStream* OaEngine::AcquireStreamOn(OaU32 InNodeIndex) {
	if (!Mesh_) {
...
void OaEngine::ReleaseStreamOn(OaU32 InNodeIndex, OaVkStream* InStream) {
	if (!Mesh_) {
		if (InNodeIndex == 0) {
...
OaStatus OaEngine::SubmitToNodeQueue(
...
void OaEngine::ReleaseMeshDemoAuxBuffer() {
...
OaStatus OaEngine::EnsurePipelineOnNode(
	OaU32 InNodeIndex,
	OaStringView InName,
	OaSpan<const OaU8> InSpirv,
	const OaPipelineSpec& InSpec){
...
OaResult<OaVkBuffer> OaEngine::AllocBufferOnNode(OaU32 InNodeIndex, OaU64 InSize) {
	if (InNodeIndex == 0) {
	...
	if (RegisterBufferOnMeshNode(*node, *res) == OA_BINDLESS_INVALID) {
		node->Allocator.Free(*res);
		OaVkLoadDevice(static_cast<VkDevice>(Device.Device));
		return OaStatus::Error(OaStatusCode::ResourceExhausted,
			"AllocBufferOnNode: bindless buffer heap exhausted");
	}
	OaVkLoadDevice(static_cast<VkDevice>(Device.Device));
	return res;
}

void OaEngine::FreeBufferOnNode(OaVkBuffer& InOutBuffer) {
...
OaStatus OaEngine::PulseAuxiliaryMeshDemoCompute() {
	if (!IsMultiDevice()) {
		return OaStatus::Ok();
	}
