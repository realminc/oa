...
OaStatus OaExecutionPlan::Compile(OaEngine& InEngine) {
	if (Compiled_) {
		return OaStatus::Error(OaStatusCode::FailedPrecondition, "execution plan is already compiled");
	}
...
OaStatus OaExecutionPlan::Replay() {
	if (not Compiled_ or Engine_ == nullptr) {
		return OaStatus::Error(OaStatusCode::FailedPrecondition, "execution plan replay called before compile");
	}
...
OaResult<OaCompletionToken> OaExecutionPlan::ReplayAsync() {
	if (not Compiled_ or Engine_ == nullptr) {
		return OaStatus::Error(OaStatusCode::FailedPrecondition, "execution plan asynchronous replay called before compile");
	}
