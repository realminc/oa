...
OaMatrix OaFlowTransformer::ForwardImpl(
	...
	if (InTokenMask) {
		...
		if ((!mask2 && !mask3) || InTokenMask->GetDtype() != InTokens.GetDtype()) {
			throw std::invalid_argument(
				"FlowTransformer token mask must match [B,S] or [B,S,1] and token dtype");
		}
		// Valid=1 -> 0, padding=0 -> a large negative additive score. Expand
		// only the compact key axis across H*S query rows once per forward.
