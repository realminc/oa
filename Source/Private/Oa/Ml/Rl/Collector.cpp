...
OaStatus OaRlCollector::Collect(OaRlRolloutBuffer& InOutRollout) {
	...
	if (InOutRollout.Size() != 0 || InOutRollout.IsFinalized()) {
		InOutRollout.Reset();
	}
	...
	OA_RETURN_IF_ERROR(InOutRollout.Finalize(Config_.Gae));
