#!/usr/bin/env python3
"""Smoke tests for the public :mod:`oa.audio` Python API.
...
def _make_sine(freq: float = 440.0) -> "core.OaMatrix":
    ...
    return core.FromFloats(data, 1, NUM_SAMPLES)
...
def test_audio_import_surface():
    for name in (
        "OaAudioDecoder", "OaAudioEncoder", "OaAudioDecodeResult", "OaAudioMeta",
...
def test_channel_layout_helpers():
    assert audio.OaChannelsForLayout(audio.OaChannelLayout.Stereo) == 2
    assert audio.OaLayoutForChannels(1) == audio.OaChannelLayout.Mono
    assert audio.OaLayoutForChannels(2) == audio.OaChannelLayout.Stereo
...
def test_mel_spectrogram_shape(engine):
    x = _make_sine()
    cfg = audio.OaMelConfig()
    cfg.FftSize = 512
    cfg.HopSize = 160
    cfg.NumMels = 40
    with oa.Context():
        mel = audio.MelSpectrogram(x, SAMPLE_RATE, cfg)
    shape = mel.Shape()
...
def test_stft_shape(engine):
    x = _make_sine()
    cfg = audio.OaStftConfig()
    cfg.FftSize = 512
    cfg.HopSize = 160
    cfg.WinSize = 512
    with oa.Context():
        spec = audio.Stft(x, cfg)
    shape = spec.Shape()
...
def test_signal_ops_shapes(engine):
    x = _make_sine()
    with oa.Context():
        gained = audio.Gain(x, -6.0)
        clipped = audio.Clip(x, -0.5, 0.5)
        normalized = audio.Normalize(x, -3.0, 0)
        mono = audio.ToMono(x)
        faded = audio.Fade(x, 128, 128)
    assert gained.Shape() == [1, NUM_SAMPLES]
    assert clipped.Shape() == [1, NUM_SAMPLES]
    assert normalized.Shape() == [1, NUM_SAMPLES]
    assert mono.Shape() == [1, NUM_SAMPLES]
    assert faded.Shape() == [1, NUM_SAMPLES]


def test_resample_length(engine):
    x = _make_sine()
    with oa.Context():
        out = audio.Resample(x, SAMPLE_RATE, SAMPLE_RATE * 2, 64)
    # Upsample 2x → roughly double the sample count.
    assert out.Shape()[1] == NUM_SAMPLES * 2


def test_clip_bounds_values(engine):
    x = _make_sine()
    with oa.Context():
        clipped = audio.Clip(x, -0.1, 0.1)
    host = core.CopyToHost(clipped)
...
def test_wav_encode_decode_roundtrip(engine, tmp_path):
    ...
    wav_bytes = audio.OaAudioEncoder.EncodeWavF32(x, SAMPLE_RATE)
    assert isinstance(wav_bytes, bytes) and len(wav_bytes) > 44  # header + samples

    path = str(tmp_path / "tone.wav")
    audio.OaAudioEncoder.SaveWavF32(path, x, SAMPLE_RATE)

    decoded = audio.OaAudioDecoder.LoadFile(path)
    ...
    meta = decoded.Meta()
    assert meta.Layout == audio.OaChannelLayout.Mono
    assert abs(meta.DurationSeconds() - NUM_SAMPLES / SAMPLE_RATE) < 1e-6

    # LoadMemory on the encoded bytes must agree with LoadFile.
    from_mem = audio.OaAudioDecoder.LoadMemory(wav_bytes)
