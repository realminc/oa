...
void BindAudioCodec(nb::module_& m) {
    ...
    nb::class_<OaAudioDecoder>(m, "OaAudioDecoder")
        .def_static("LoadFile", [](nb::handle path) {
            // Decoding uploads the planar samples into an OaMatrix. Ensure the
            // process engine exists before the C++ codec reaches that upload.
            (void)PythonEngine();
            auto result = OaAudioDecoder::LoadFile(path_from_python(path));
            throw_if_error(result.GetStatus());
            return new OaAudio(std::move(result).GetValue());
    ...
    nb::class_<OaAudioEncoder>(m, "OaAudioEncoder")
        .def_static("EncodeWavF32", [](const OaAudio& audio) {
            // Executes pending GPU work, reads planar F32, interleaves, encodes.
            auto result = OaAudioEncoder::EncodeWavF32(audio);
            throw_if_error(result.GetStatus());
            auto& blob = result.GetValue();
            return nb::bytes(blob.Data(), blob.Size());
        }, nb::arg("Audio"),
           "Encode an OaAudio value to WAV-F32 bytes (synchronous).")
        .def_static("SaveWavF32", [](nb::handle path, const OaAudio& audio) {
            throw_if_error(OaAudioEncoder::SaveWavF32(
                path_from_python(path), audio));
