...
/// Interpolated state and constant path velocity from one linear-match step.
#[pyclass(name = "FlowMatchBatch", unsendable)]
pub(crate) struct PythonFlowMatchBatch {
	inner: oa::ml::flow::FlowMatchBatch,
}

#[pymethods]
impl PythonFlowMatchBatch {
	/// Interpolated state `x(t)`.
	#[getter]
	pub fn state(&self) -> PythonMatrix {
		PythonMatrix::wrap(self.inner.state.clone())
	}

	/// Constant linear-path velocity `noise - clean`.
	#[getter]
	pub fn velocity(&self) -> PythonMatrix {
		PythonMatrix::wrap(self.inner.velocity.clone())
	}
...
#[pyfunction]
pub(crate) fn ml_flow_linear_match(
	clean: &PythonMatrix,
	noise: &PythonMatrix,
	time: &PythonMatrix,
) -> PyResult<PythonFlowMatchBatch> {
	oa::ml::flow::linear_match(&clean.inner, &noise.inner, &time.inner)
		.map(|inner| PythonFlowMatchBatch { inner })
		.map_err(python_error)
}

#[pyfunction]
pub(crate) fn ml_flow_euler_step(
	state: &PythonMatrix,
	velocity: &PythonMatrix,
	delta_time: f32,
) -> PyResult<PythonMatrix> {
	oa::ml::flow::euler_step(&state.inner, &velocity.inner, delta_time)
		.map(PythonMatrix::wrap)
		.map_err(python_error)
}

#[pyfunction]
pub(crate) fn ml_flow_masked_mse(
	prediction: &PythonMatrix,
	target: &PythonMatrix,
	mask: &PythonMatrix,
) -> PyResult<PythonMatrix> {
	oa::ml::flow::masked_mse(&prediction.inner, &target.inner, &mask.inner)
		.map(PythonMatrix::wrap)
		.map_err(python_error)
}

pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
	module.add_class::<PythonFlowMatchBatch>()?;
