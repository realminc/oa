...
use super::{BpeTokenizer, Module, ModuleRegistry, nn};
...
pub const BPE_MERGES: usize = BPE_VOCAB_SIZE - super::byte::VOCAB_SIZE;
		...
		for batch in 0..self.batch_size {
			...
			for position in 0..CONTEXT_LENGTH {
				...
				self.last_batch_bytes = self
					...
					.ok_or_else(|| {
						Error::resource_exhausted("NLP BPE byte count overflows usize")
					})?;
			}
		}
		self.cursor = (self.cursor + self.batch_size) % limit;
...
pub fn generate_bytes_greedy(
	...
	for _ in 0..count {
		let input = Matrix::from_slice(engine, [1, CONTEXT_LENGTH], &context)?;
		let logits = model.forward(&input)?;
		if logits.shape() != [CONTEXT_LENGTH, super::byte::VOCAB_SIZE] {
			return Err(Error::failed_precondition(
		...
		let row = crate::matrix::slice(&logits, 0, logit_row as i64, (logit_row + 1) as i64)?
			.reshape([super::byte::VOCAB_SIZE])?;
		let next = super::byte::decode(&row)?[0];
		output.push(next);
		if filled < CONTEXT_LENGTH {
			context[filled] = next;
			filled += 1;
...
pub fn generate_bpe_greedy(
	...
	for _ in 0..byte_count {
		...
		generated_bytes = generated_bytes
			...
			.ok_or_else(|| {
				Error::resource_exhausted("BPE generation byte count overflows usize")
			})?;
		if filled < CONTEXT_LENGTH {
			context[filled] = next;
			filled += 1;
