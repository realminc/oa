//! Finite planar audio clips layered over [`crate::Matrix`].
...
pub struct Audio {
	...
	semantic: Rc<AudioSemantic>,
}

/// Persistent semantic identity shared by clones of one Audio value.
pub(crate) struct AudioSemantic {
	id: u64,
}
...
impl Audio {
	...
	pub fn new(data: Matrix, sample_rate: u32, layout: AudioChannelLayout) -> Result<Self> {
		...
		Ok(Self {
			...
			sample_rate,
			layout,
			semantic: Rc::new(AudioSemantic {
				id: next_audio_value_id()?,
			}),
		})
	}
...
fn next_audio_value_id() -> Result<u64> {
	static NEXT_AUDIO_VALUE_ID: AtomicU64 = AtomicU64::new(1);
	let id = NEXT_AUDIO_VALUE_ID.fetch_add(1, Ordering::Relaxed);
	if id == u64::MAX {
		return Err(Error::resource_exhausted(
			"audio semantic value identity space is exhausted",
		));
	}
	Ok(id)
}
