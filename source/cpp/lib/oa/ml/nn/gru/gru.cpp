...
oa::Matrix oa::Gru::forward(const oa::Matrix& inInput) {
	...
	// slicing the wrong axis into O(seq^2) buffers with garbage gradients. Guard
	// it in every build.
	if (inInput.rank() != 3) {
		OaLogError(oa::LogComponent::Ml,
			"oa::Gru::forward expects rank-3 [batch, seq, input], got rank={} — "
			"reshape the embedding output (Gather flattens to [batch*seq, embed])",
			inInput.rank());
	}
	OA_REQUIRE_MSG(inInput.rank() == 3,
		"oa::Gru::forward requires rank-3 [batch, seq, input]");
