	...
	, config_(inConfig)
{
	...
	config_.hSpacing = std::max(0, config_.hSpacing);
	config_.vSpacing = std::max(0, config_.vSpacing);
	config_.padding = std::max(0, config_.padding);
...
CellLayout resolveCellLayout(
	...
	layout.title = {inCell.x + yLabelBand, inCell.y,
		std::max(0, inCell.w - 2 * yLabelBand), inHasTitle ? titleHeight : 0};
	layout.yLabel = {inCell.x, inCell.y + titleBand,
		inHasYLabel ? yLabelWidth : 0,
		std::max(0, inCell.h - titleBand - xLabelBand - captionBand)};
	layout.plot = {inCell.x + yLabelBand, inCell.y + titleBand,
		std::max(0, inCell.w - 2 * yLabelBand),
		std::max(0, inCell.h - titleBand - xLabelBand - captionBand)};
...
struct DataBounds {
	oa::F32 xMin = std::numeric_limits<oa::F32>::infinity();
	oa::F32 xMax = -std::numeric_limits<oa::F32>::infinity();
	oa::F32 yMin = std::numeric_limits<oa::F32>::infinity();
	oa::F32 yMax = -std::numeric_limits<oa::F32>::infinity();

	void include(oa::F32 inX, oa::F32 inY) noexcept {
		if (not std::isfinite(inX) or not std::isfinite(inY)) return;
		xMin = std::min(xMin, inX);
		xMax = std::max(xMax, inX);
		yMin = std::min(yMin, inY);
		yMax = std::max(yMax, inY);
	}
	[[nodiscard]] bool isValid() const noexcept {
		return std::isfinite(xMin) and std::isfinite(xMax)
			and std::isfinite(yMin) and std::isfinite(yMax);
	}
	void expandDegenerate() noexcept {
		if (xMax <= xMin) {
			const oa::F32 margin = std::max(1.0e-4F, std::abs(xMin) * 0.05F);
			xMin -= margin;
			xMax += margin;
		}
		if (yMax <= yMin) {
...
Figure::Rect Figure::cellRect(oa::I32 inRow, oa::I32 inCol,
	...
	const oa::I32 gridW = std::max(0, frame.w - 2 * pad - 2 * yLabelW);
	const oa::I32 gridH = std::max(0, frame.h - 2 * pad - titleH - xLabelH);
	const oa::I32 cellW = std::max(
		0, (gridW - (config_.cols - 1) * hgap) / config_.cols);
	const oa::I32 cellH = std::max(
...
void Figure::renderFrame(oa::U32 inWidth, oa::U32 inHeight, ::oa::Ui& inUi) {
	...
	const auto drawText = [&](oa::StringView inText, Rect inRect,
		...
		inUi.textAt(inText, {inRect.x, inRect.y, inRect.w, inRect.h}, {
			.fontSize = std::max(1.0F, inSize * frame.scale),
			.color = inColor,
	...
	oa::I32 fixedImageSize = std::numeric_limits<oa::I32>::max();
	...
	for (oa::I32 row = 0; row < rows(); ++row) {
		for (oa::I32 column = 0; column < cols(); ++column) {
			...
			const CellLayout layout = resolveCellLayout(
				...
				frame.scale);
			const oa::I32 candidate = std::min(layout.plot.w, layout.plot.h);
			if (candidate <= 0) continue;
			fixedImageSize = std::min(fixedImageSize, candidate);
			hasSquareImage = true;
		}
	}
		...
		for (oa::I32 column = 0; column < cols(); ++column) {
			...
			inUi.grid(
				{layout.plot.x, layout.plot.y, layout.plot.w, layout.plot.h}, {
					...
					.minorSpacing = {
						std::max(4.0F, static_cast<oa::F32>(layout.plot.w - 1) / 100.0F),
						std::max(4.0F, static_cast<oa::F32>(layout.plot.h - 1) / 100.0F),
					},
					.drawGrid = false,
					.drawAxes = false,
				});
			if (axes.heatmap_.present and not axes.heatmap_.v.empty()) {
				...
				if (axes.heatmap_.style.autoScale) {
					minimum = std::numeric_limits<oa::F32>::infinity();
					maximum = -std::numeric_limits<oa::F32>::infinity();
					for (oa::F32 value : axes.heatmap_.v) {
						if (not std::isfinite(value)) continue;
						minimum = std::min(minimum, value);
						maximum = std::max(maximum, value);
					}
				}
				if (std::isfinite(minimum) and std::isfinite(maximum)) {
					if (maximum <= minimum) maximum = minimum + 1.0F;
					std::snprintf(id, sizeof(id), "figure-heat-%d-%d", row, column);
				...
				std::snprintf(id, sizeof(id), "figure-image-%d-%d", row, column);
			...
			if (not axes.artists_.empty()) {
				...
				if (bounds.isValid() and bounds.xMax > bounds.xMin
					...
					const auto mapX = [&](oa::F32 inX) {
						return static_cast<oa::F32>(layout.plot.x)
							+ (inX - bounds.xMin) / (bounds.xMax - bounds.xMin)
								* static_cast<oa::F32>(std::max(0, layout.plot.w - 1));
					};
					const auto mapY = [&](oa::F32 inY) {
						return static_cast<oa::F32>(layout.plot.y)
							+ (1.0F - (inY - bounds.yMin)
								/ (bounds.yMax - bounds.yMin))
								* static_cast<oa::F32>(std::max(0, layout.plot.h - 1));
					};
					std::snprintf(id, sizeof(id), "figure-artists-%d-%d", row, column);
					inUi.beginPanel(id, {layout.plot.x, layout.plot.y,
						layout.plot.w, layout.plot.h});
					if (axes.showGrid_) {
						...
						const oa::F32 shortSide = static_cast<oa::F32>(std::max(
							1, std::min(layout.plot.w - 1, layout.plot.h - 1)));
						...
						switch (artist.kind) {
							case Axes::ArtistKind::Line: {
								...
								const bool direct = line.x.empty()
									and std::abs(bounds.xMin) <= 1.0e-5F
									and std::abs(bounds.xMax - implicitMaximum) <= 1.0e-5F;
								if (direct) {
									std::snprintf(id, sizeof(id),
								...
								if (line.y.size() == 1U) {
									const oa::F32 x = 0.0F;
									if (std::isfinite(x) and std::isfinite(line.y[0])) {
										inUi.rect({static_cast<oa::I32>(std::lround(mapX(x))) - 2,
											static_cast<oa::I32>(std::lround(mapY(line.y[0]))) - 2,
											5, 5}, color);
									}
									break;
								}
								for (oa::Usize i = 1U; i < line.y.size(); ++i) {
									...
									if (not std::isfinite(x0) or not std::isfinite(x1)
										or not std::isfinite(line.y[i - 1U])
										or not std::isfinite(line.y[i])) continue;
									inUi.line({mapX(x0), mapY(line.y[i - 1U])},
										{mapX(x1), mapY(line.y[i])}, color,
										std::clamp(line.style.width * frame.scale, 0.5F, 16.0F));
								}
								break;
							}
							case Axes::ArtistKind::Scatter: {
								...
								const oa::I32 radius = static_cast<oa::I32>(std::lround(
									std::clamp(scatter.style.radius * frame.scale, 1.0F, 12.0F)));
								for (oa::Usize i = 0U; i < scatter.y.size(); ++i) {
									if (not std::isfinite(scatter.x[i])
										or not std::isfinite(scatter.y[i])) continue;
									inUi.rect({
										static_cast<oa::I32>(std::lround(mapX(scatter.x[i]))) - radius,
										static_cast<oa::I32>(std::lround(mapY(scatter.y[i]))) - radius,
							...
							case Axes::ArtistKind::Bar: {
								...
								for (oa::Usize i = 0U; i < bar.y.size(); ++i) {
									if (not std::isfinite(bar.x[i])
										or not std::isfinite(bar.y[i])) continue;
									...
									const oa::I32 x = static_cast<oa::I32>(std::floor(std::min(left, right)));
									const oa::I32 y = static_cast<oa::I32>(std::floor(std::min(baseline, value)));
									const oa::I32 w = std::max(1, static_cast<oa::I32>(std::ceil(std::abs(right - left))));
									const oa::I32 h = std::max(1, static_cast<oa::I32>(std::ceil(std::abs(value - baseline))));
					...
					if (axes.showLegend_) {
						...
						if (labeled > 0U) {
							const oa::I32 rowHeight = std::max(14, scaledLayoutValue(18, frame.scale));
							const oa::I32 legendWidth = std::min(layout.plot.w - 8,
								std::max(96, scaledLayoutValue(150, frame.scale)));
							const oa::I32 legendHeight = std::min(layout.plot.h - 8,
							...
							const oa::F32 legendRadius = std::max(
							...
							for (const Axes::ArtistRef& artist : axes.artists_) {
								...
								inUi.textAt(label,
									{legendX + 34, legendY + 4 + static_cast<oa::I32>(labelIndex) * rowHeight,
										...
										.fontSize = std::max(9.0F, 11.0F * frame.scale),
...
oa::Status Figure::renderRgba(
	...
	if (config_.width == 0U or config_.height == 0U
		or config_.width > static_cast<oa::U32>(std::numeric_limits<oa::I32>::max())
		or config_.height > static_cast<oa::U32>(std::numeric_limits<oa::I32>::max())) {
		return oa::Status::invalidArgument(
