...
// Module API: OaModule + OaLinear + OaAdamW + OaFnLoss + OaGradientTape
// ═══════════════════════════════════════════════════════════════════════════
//
// Sibling of TutorialMnistClassifierApi1.cpp (manual backward — the speed-of-
// light reference). This variant uses *implicit* autograd: OaGradientTape
// attaches a tape on each forward op; tape.Backward(loss) walks the tape and
// records the matching *Bwd kernels into the same OaContext. No hand-written
...
// No manual Backward — OaGradientTape walks the tape that OaLinear/Scale/
...
public:
	...
	OaMatrix Forward(const OaMatrix& InX) override {
		auto x = OaFnMatrix::Scale(InX, 1.0F / 255.0F);  // tape: OaGradScale
		auto h = Fc1_->Forward(x);                       // tape: OaGradLinearRelu
		return Fc2_->Forward(h);                         // tape: OaGradLinear
	}
...
TEST(TutorialMnistClassifierAg, FashionMnistClassification) {
	...
	OaDsMnist trainLoader(OaString(dataDir), "train", kBatch, /*shuffle=*/true);
