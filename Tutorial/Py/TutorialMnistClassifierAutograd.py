...
class MnistDataLoader:
    ...
    def next_batch(self):
        ...
        # Convert to OaMatrix
        x = core.FromFloats(batch_images, actual_batch, INPUT_DIM)
        y = core.FromBytes(batch_labels, actual_batch, core.OaScalarType.UInt8)
...
class OaMnistClassifier:
    def __init__(self):
        ...
        self.fc1 = ml.OaLinear(INPUT_DIM, HIDDEN_DIM)
        self.fc1.SetActivation(ml.OaActivation.Relu)
        self._fc1_p = self.fc1.Parameters()
        self._fc1_p[0].Data = core.RandKaimingUniform(HIDDEN_DIM, INPUT_DIM, wd)
        self._fc1_p[0].Data.SetRequiresGrad(True)
        self._fc1_p[1].Data.SetRequiresGrad(True)
        
        # Output layer
        self.fc2 = ml.OaLinear(HIDDEN_DIM, NUM_CLASSES)
        self._fc2_p = self.fc2.Parameters()
        self._fc2_p[0].Data = core.RandGlorotUniform(NUM_CLASSES, HIDDEN_DIM, wd)
    ...
    def forward(self, x):
        ...
        x_norm = core.Scale(x, 1.0 / 255.0)  # Normalize [0,255] → [0,1]
