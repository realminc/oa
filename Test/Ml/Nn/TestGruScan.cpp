...
// Build an OaGru filled with deterministic weights (identical across calls w/ same phase).
// OaGru::Parameters() is non-recursive (empty: the cell is a submodule), so we must
// fill via AllParameterPtrs() — otherwise ref/scan get different RandXavier weights.
static OaSharedPtr<OaGru> MakeGru(OaI32 InInput, OaI32 InHidden) {
	auto gru = OaMakeSharedPtr<OaGru>(InInput, InHidden, 1, true);
	double phase = 0.0;
