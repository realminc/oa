...
The accepted local Iris Xe Release run produced:
...
the complete batch. This run is correctness evidence, not a performance-parity
claim.
...
captures the 71-node forward/backward/AdamW step once, reuses one recorded
Vulkan command across all 300 submissions, uploads 299 new input/target pairs
into stable slots without rerecording, and checks 299 command cache hits. It
...
| OARS optimized kernels + captured replay | 4.040804 | 0.193185 | 92.4% | 14.674 ms/step |
...
The differing C++/Rust initial loss reflects different deterministic
initializer streams; it is not stepwise numerical identity. The current OARS
run recorded 71 nodes once, 299 cache hits, and 598 stable input uploads. Its
wall result is an immediate exploratory run under uncontrolled thermal state,
not the fixed-clock seven-process protocol, so it is not yet a qualified
performance comparison with the C++ row. Final quality and greedy behavior
match exactly.
...
The shared training/callback lifecycle, fixed-clock fresh-process performance
qualification, and the remaining Byte/BPE ×
RNN/GRU/Transformer/MoE/Mamba-3 rows remain Planned.
