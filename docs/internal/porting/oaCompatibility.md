...
| ML inference | Preserve progressively | Matrix/GEMM, recurrent, attention, MoE, and the complete parameter-owning FP32 Mamba-3 SISO/MIMO paths are connected. The SDK-owned ALM core adds a temporal Conv1d VQ-VAE, dense/MoE/hybrid causal token prior, native frozen CLIP text tower and byte-BPE parser, autoregressive generation, motion decode, exact SafeTensors-to-CLIP translation, donor-compatible CLIP v1 model files, ALM v3 product bundles through one `.oam` codec, HumanML3D/CMP/KIT-ML host data plus geometry metrics, deterministic tokenizer/prior windows, both native stage trainers, end-to-end dataset orchestration, held-out epoch validation with the donor tokenizer/prior metric sets, atomic per-stage model/optimizer/progress resume, and runnable training/NPY-generation applications. Conditioned `ml_alm_train` now loads the pinned native CLIP tower and merge asset, bakes train/held-out captions through that frozen owner, and embeds both assets in the final bundle without requiring cached feature rows. USD preview export, sharded/additional translators, KV cache, broader dtypes, and mobile-specific providers remain gated. |
