...
| ML inference | Preserve progressively | Matrix/GEMM, recurrent, attention, MoE, and the complete parameter-owning FP32 Mamba-3 SISO/MIMO paths are connected. The SDK-owned ALM core adds a temporal Conv1d VQ-VAE, dense/MoE/hybrid causal token prior, native frozen CLIP text tower and byte-BPE parser, autoregressive generation, motion decode, exact SafeTensors-to-CLIP translation, donor-compatible CLIP v1 model files, and ALM v3 product bundles through one `.oam` codec. ALM data/training applications, sharded/additional weight translators, KV cache, broader dtypes, and mobile-specific providers remain gated. |
