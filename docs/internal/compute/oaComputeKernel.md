...
**Status:** Canonical contract; generated `f32` family and `i32` add are Experimental
...
entry-point facts. Reflection proves the compiled interface. The runtime owns
...
Every admitted physical entry point has a generated non-zero stable ID and an
...
The current IDs are local Experimental `u16` values owned by the elementwise
schema. Before multiple schema owners ship, OA must add a durable owner-prefix
or equivalent collision-free allocation ledger. Physical GEMM variants may
need a separate exact variant identity rather than pretending every tile is a
new semantic operation.

Artifact identity ultimately includes the compiled module hash, entry point,
...
Slang modules. Current generated elementwise shaders import
...
by scalar fields. Current layouts are:
...
entry point, and OA attributes. Rust records typed `StorageBuffer`, `U32`, and
`F32` push values and rejects a byte count that differs from the artifact ABI.
...
The current device eagerly creates one pipeline per generated elementwise
...
5. Compile every entry, validate reflection, and run `spirv-val`.
...
- generated Slang entry points: `src/slang/matrix/elemwise/`
