...
pub fn run<M: Module>(
	...
	println!("║  {title:<62}║");
	println!("╚══════════════════════════════════════════════════════════════════╝\n");
	println!("tokenizer: raw bytes · vocab={}", oa::ml::byte::VOCAB_SIZE);
	println!("Task: dense next-byte prediction at every position\n");
	...
	let log_dir = oa::Path::var_rel(format!("log/nlp/{timer_name}"));
	...
	println!(
		...
		oa::ml::byte::VOCAB_SIZE,
		(oa::ml::byte::VOCAB_SIZE as f64).ln()
	);
	println!("  byte-token accuracy: {:.1}%", accuracy * 100.0);
	...
	let checkpoint = oa::Path::temp().join(format!("{checkpoint_stem}-{}.oam", std::process::id()));
	...
	oa::Filesystem::remove_file(&checkpoint).expect("remove completed Byte checkpoint");
	Ok(())
}
