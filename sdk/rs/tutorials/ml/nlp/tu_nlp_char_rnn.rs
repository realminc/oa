...
fn main() -> oa::Result<()> {
	let mut cli = Cli::new("tu_nlp_char_rnn", "OA Tutorial — Char RNN");
	cli.add_option(
		"--steps",
	...
	let log_dir = oa::Path::var_rel("log/nlp/char_rnn");
	...
	let checkpoint = oa::Path::temp().join("oars_char_rnn.oam");
	...
	oa::Filesystem::remove_file(&checkpoint).expect("remove completed RNN checkpoint");
	Ok(())
}
