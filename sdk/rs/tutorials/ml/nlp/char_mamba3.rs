...
fn main() -> oa::Result<()> {
	...
	println!(
		"params: {}    Optimizer: AdamW(lr=0.003)\n",
	...
	let checkpoint = std::env::temp_dir().join("oars_char_mamba3.oam");
	oa::ml::save_checkpoint(&checkpoint, &model, &optimizer)?;
	let reloaded = nlp::CharMamba3::new(&engine)?;
	let mut reloaded_optimizer = oa::ml::AdamW::new(reloaded.all_parameters()?, 0.003)?;
	...
	std::fs::remove_file(checkpoint).expect("remove completed Mamba-3 checkpoint");
	Ok(())
}
