...
fn main() -> oa::Result<()> {
	let mut cli = Cli::new("tu_nlp_char_mamba3", "OA Tutorial — Char Mamba-3");
	cli.add_option(
		"--steps",
	...
	println!("\n╔══════════════════════════════════════════════════════════════════╗");
	println!("║  OA Tutorial — Char Mamba-3 · all-position LM (autograd)         ║");
	println!("╚══════════════════════════════════════════════════════════════════╝\n");
	println!(
	...
	println!("Model: Embedding(32) → Mamba-3 SISO → residual → Linear(27)");
	println!(
	...
	let mut training = ItTraining::new(
		...
		ItTrainingConfig {
			...
			timer_name: "char_mamba3_training_step".into(),
	...
	let log_dir = oa::Path::var_rel("log/nlp/char_mamba3");
	...
	if steps == nlp::TRAINING_STEPS && batch == nlp::BATCH_SIZE && lr == 0.003 {
		assert!(initial_loss > 3.0);
		assert!((evaluation_loss - nlp::CHAR_MAMBA3_FINAL_LOSS).abs() < 0.000_01);
		assert!((accuracy - nlp::CHAR_MAMBA3_ACCURACY).abs() < 0.000_001);
		assert_eq!(generated, nlp::CHAR_MAMBA3_REFERENCE_GENERATION);
	}

	let checkpoint = oa::Path::temp().join("oars_char_mamba3.oam");
	oa::ml::save_checkpoint(&checkpoint, &model, &optimizer)?;
	let reloaded = nlp::CharMamba3::new(&engine)?;
	...
	oa::Filesystem::remove_file(&checkpoint).expect("remove completed Mamba-3 checkpoint");
	Ok(())
}
