...
static oa::NlpArchitecture OaMobileParseArchitecture(const std::string& inValue) {
	if (inValue == "rnn") {
...
static oa::NlpTokenizerKind OaMobileParseTokenizer(const std::string& inValue) {
	if (inValue == "bpe") {
...
static std::string OaMobileEscapeText(const oa::String& inValue) {
...
static OaMobileGenerationQuality OaMobileMeasureGenerationQuality(
	...
	const oa::Usize promptLength = std::strlen(OaNlpSuiteGenerationPrompt);
	const oa::Usize expectedLength = promptLength +
		static_cast<oa::Usize>(OaNlpSuiteGenerationSourceUnits);
...
static oa::String OaMobileGenerateGreedy(
	...
	if (inRecipe.architecture() == oa::NlpArchitecture::Mamba3 and
		...
		oa::String output(OaNlpSuiteGenerationPrompt);
		...
		const oa::Usize targetLength = std::strlen(OaNlpSuiteGenerationPrompt) +
			static_cast<oa::Usize>(OaNlpSuiteGenerationSourceUnits);
...
static std::string OaMobileRunTraining(
	...
	const std::string deviceName(deviceNameView.data(), deviceNameView.size());

	{
		...
		oa::FnMatrix::setRngSeed(OaNlpSuiteRngSeed);
		...
		if (inResume) {
			const auto load = model->load(*engine, inCheckpointPath.c_str(), *optimizer);
			if (not load.isOk()) {
		...
		const auto save = model->save(*engine, inCheckpointPath.c_str(), *optimizer);
		if (not save.isOk()) {
		...
		if (not OaMobileCancelRequested.load(std::memory_order_acquire) and
			...
			const auto load = reloaded->load(
				*engine, inCheckpointPath.c_str(), *reloadOptimizer);
			if (not load.isOk()) {
	...
	if (not completion.isOk()) {
		OaMobileFail("Final runtime submission failed: " +
			std::string(completion.toString().c_str()));
	}
