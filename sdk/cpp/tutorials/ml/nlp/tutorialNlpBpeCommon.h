...
public:
	...
	explicit NlpBpeTokenizer(const char* inTrainingText, oa::I32 inVocabSize = kBpeVocabSize)
		: vocabSize_(oa::max<oa::I32>(inVocabSize, 256)) {
		vocab_.resize(static_cast<oa::Usize>(vocabSize_));
		for (oa::U32 i = 0; i < 256; ++i) vocab_[i].pushBack(static_cast<oa::U8>(i));

		oa::Vector<oa::U32> ids;
		const auto* bytes = reinterpret_cast<const oa::U8*>(inTrainingText);
		for (oa::Usize i = 0; i < oa::strlen(inTrainingText); ++i) ids.pushBack(bytes[i]);

		for (oa::U32 token = 256; token < static_cast<oa::U32>(vocabSize_) && ids.size() > 1; ++token) {
			oa::HashMap<oa::U64, oa::I64> counts;
			for (oa::Usize i = 0; i + 1 < ids.size(); ++i) {
				const oa::U64 key = pairKey(ids[i], ids[i + 1]);
				auto found = counts.find(key);
				if (found == counts.end()) counts.emplace(key, 1);
				else ++found->second;
			}
			oa::U64 bestKey = oa::Limits<oa::U64>::max();
			oa::I64 bestCount = -1;
	...
	[[nodiscard]] oa::Vector<oa::U32> encode(const char* inText) const {
		oa::Vector<oa::U32> ids;
		const auto* bytes = reinterpret_cast<const oa::U8*>(inText);
		for (oa::Usize i = 0; i < oa::strlen(inText); ++i) ids.pushBack(bytes[i]);
	...
	[[nodiscard]] oa::String decode(const oa::Vector<oa::U32>& inIds) const {
		oa::String out;
...
private:
	static oa::U64 pairKey(oa::U32 inLeft, oa::U32 inRight) {
		return (static_cast<oa::U64>(inLeft) << 32) | inRight;
	}

	static oa::Vector<oa::U32> applyMerge(const oa::Vector<oa::U32>& inIds,
		const Merge& inMerge, oa::U32 inToken) {
		oa::Vector<oa::U32> out;
		out.reserve(inIds.size());
		for (oa::Usize i = 0; i < inIds.size();) {
			if (i + 1 < inIds.size() && inIds[i] == inMerge.left && inIds[i + 1] == inMerge.right) {
				out.pushBack(inToken);
				i += 2;
			} else {
	...
	oa::Vector<oa::Vector<oa::U8>> vocab_;
};
...
public:
	...
	void nextBatch(oa::Matrix& outX, oa::Matrix& outY) {
		oa::Vector<oa::U32> x(static_cast<oa::Usize>(batchSize_) * kContextLen);
		oa::Vector<oa::U32> y(static_cast<oa::Usize>(batchSize_) * kContextLen);
		...
		for (oa::I32 b = 0; b < batchSize_; ++b) {
			...
			for (oa::I32 t = 0; t < kContextLen; ++t) {
				const oa::Usize dst = static_cast<oa::Usize>(b) * kContextLen + t;
				x[dst] = tokens_[static_cast<oa::Usize>(start + t)];
				y[dst] = tokens_[static_cast<oa::Usize>(start + t + 1)];
...
private:
	oa::Matrix fromU32(const oa::Vector<oa::U32>& inIds) const {
...
oa::String nlpGenerateBpeGreedy(Model& inModel, const NlpBpeTokenizer& inTokenizer,
	...
	oa::Vector<oa::U32> context(kContextLen, 0);
	const oa::I32 copyCount = oa::min<oa::I32>(static_cast<oa::I32>(prompt.size()), kContextLen);
	for (oa::I32 i = 0; i < copyCount; ++i) context[static_cast<oa::Usize>(i)] = prompt[static_cast<oa::Usize>(i)];
	oa::I32 filled = oa::max(copyCount, 1);
	oa::I32 logitRow = filled - 1;
	oa::Vector<oa::U32> generated = prompt;
	...
	for (oa::I32 i = 0; i < inByteCount and generatedBytes < inByteCount; ++i) {
		...
		if (filled < kContextLen) {
			...
			for (oa::I32 t = 1; t < kContextLen; ++t) {
				context[static_cast<oa::Usize>(t - 1)] = context[static_cast<oa::Usize>(t)];
			}
			context.back() = next;
	...
	const oa::Usize targetBytes = oa::strlen(inPrompt) + static_cast<oa::Usize>(inByteCount);
