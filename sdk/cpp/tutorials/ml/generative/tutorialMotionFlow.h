...
inline oa::F32 learningRateFromEnvironment() {
	...
	const oa::F32 parsed = std::strtof(value, nullptr);
...
inline BatchData nextBatch(oa::DsCombatMotionProcessed& inDataset, oa::I32& inOutCursor) {
	...
	for (oa::I32 batch = 0; batch < kBatch; ++batch) {
		...
		const oa::I32 frames = std::min(kSequence, inDataset.clipFrames(clip));
		if (frames > 0) {
			std::memcpy(
			...
			std::fill(mask.begin() + static_cast<oa::I64>(batch) * kSequence,
				mask.begin() + static_cast<oa::I64>(batch) * kSequence + frames, 1.0F);
		}
		if (conditionDim > 0 && inDataset.clipTextFeatureCount(clip) > 0) {
			std::memcpy(condition.data() + static_cast<oa::I64>(batch) * conditionDim,
	...
	for (oa::I32 batch = 0; batch < kBatch; ++batch) {
		...
		result.lengths[batch] = std::min(kSequence, inDataset.clipFrames(clip));
	}
	return result;
}
...
inline void validateGeometryAndExport(
	...
	for (oa::F32 coordinate : world) ASSERT_TRUE(std::isfinite(coordinate));
