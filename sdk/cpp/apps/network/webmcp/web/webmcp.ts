...
interface SiteTool {
  ...
  execute: (argumentsValue: JsonObject, options?: ToolExecuteOptions) => Promise<unknown>;
}
...
interface VulkanStatus extends JsonObject {
  deviceName: string;
  deviceVendor: string;
  deviceType: string;
  driverName: string;
  ...
  hasCompute: boolean;
  hasGraphics: boolean;
  deviceVramBytes: number;
  memoryTotalBytes: number;
  memoryUsedBytes: number;
  memoryFreeBytes: number;
}

interface TrainingStatus extends JsonObject {
  runId: number;
  revision: number;
  state: string;
  step: number;
...
interface TrainingGeneration extends JsonObject {
  checkpointRoundTrip: boolean;
  generationQualityPassed: boolean;
  accuracy: number;
...
function isToolCallResult(value: unknown): value is ToolCallResult {
  ...
  if (value.structuredContent !== undefined && !isObject(value.structuredContent)) return false;
  if (value.content !== undefined && (!Array.isArray(value.content) ||
      !value.content.every((item) => isObject(item) && typeof item.type === "string" &&
        (item.text === undefined || typeof item.text === "string")))) return false;
  return true;
}

function isVulkanStatus(value: unknown): value is VulkanStatus {
  if (!isObject(value)) return false;
  ...
  return strings.every((name) => typeof value[name] === "string") &&
    numbers.every((name) => typeof value[name] === "number" && Number.isFinite(value[name]) && (value[name] as number) >= 0) &&
...
function isTrainingStatus(value: unknown): value is TrainingStatus {
  ...
  return integers.every((name) => typeof value[name] === "number" &&
      Number.isSafeInteger(value[name]) && (value[name] as number) >= 0) &&
    ...
    (value.runId as number) >= 1 && (value.totalSteps as number) >= 1;
}

function isTrainingGeneration(value: unknown): value is TrainingGeneration {
...
let lastTrainingStatus: TrainingStatus | null = null;
let renderedRunId = 0;
...
function setBadge(id: string, text: string, state: "ready" | "unavailable" | "error"): void {
...
async function mcpRequest<T>(
  ...
  if (!response.ok) throw new Error(`native gateway returned HTTP ${response.status}`);
  const message = await response.json() as JsonRpcResponse<T>;
  if (message.error) throw new Error(`MCP ${message.error.code}: ${message.error.message}`);
  if (message.result === undefined) throw new Error("MCP response omitted result");
...
async function callTool(
  ...
  if (!allowedTools.has(name)) throw new Error(`tool is not allowlisted: ${name}`);
  const result = await mcpRequest<unknown>("tools/call", {
    name,
    arguments: argumentsValue,
  }, signal);
  if (!isToolCallResult(result)) throw new Error("native MCP tool returned an invalid result shape");
  return result;
}

async function callVulkanStatus(argumentsValue: JsonObject = {}): Promise<ToolCallResult> {
...
function statusFromResult(result: ToolCallResult): VulkanStatus {
  if (result.isError) {
    throw new Error(result.content?.[0]?.text ?? "native MCP tool failed");
  }
  const status = result.structuredContent;
  if (!isVulkanStatus(status)) throw new Error("vulkan_status returned invalid structuredContent");
  return status;
}

function structuredFromResult(result: ToolCallResult): JsonObject {
  if (result.isError) {
    throw new Error(result.content?.[0]?.text ?? "native MCP tool failed");
  }
...
function trainingStatusFromResult(result: ToolCallResult): TrainingStatus {
  const status = structuredFromResult(result);
  if (!isTrainingStatus(status)) throw new Error("training_status returned invalid structuredContent");
  return status;
}

function renderVulkan(result: ToolCallResult): void {
...
function renderTraining(result: ToolCallResult): TrainingStatus {
  ...
  if (renderedRunId !== status.runId) {
    ...
    (element("config-total-steps") as HTMLInputElement).value = String(status.totalSteps);
    (element("config-model-width") as HTMLInputElement).value = String(status.modelWidth);
    (element("config-hidden-width") as HTMLInputElement).value = String(status.hiddenWidth);
    (element("config-context-length") as HTMLInputElement).value = String(status.contextLength);
    (element("config-batch-size") as HTMLInputElement).value = String(status.batchSize);
    if (status.learningRate !== null) {
      (element("config-learning-rate") as HTMLInputElement).value = String(status.learningRate);
      (element("learning-rate") as HTMLInputElement).value = String(status.learningRate);
    }
  }
  ...
  (element("training-progress") as HTMLElement).style.width = `${Math.min(
    100,
    status.totalSteps === 0 ? 0 : (status.step / status.totalSteps) * 100,
  )}%`;
  const start = element("start-training") as HTMLButtonElement;
  ...
  (element("pause-training") as HTMLButtonElement).disabled = status.state !== "running";
  (element("resume-training") as HTMLButtonElement).disabled =
    status.state !== "paused" || status.step === 0;
  const active = status.state === "running" || status.state === "paused";
  (element("checkpoint-training") as HTMLButtonElement).disabled = !active;
  (element("evaluate-training") as HTMLButtonElement).disabled = !active;
  (element("apply-learning-rate") as HTMLButtonElement).disabled =
    !active;
  (element("read-generation") as HTMLButtonElement).disabled = status.state !== "completed";
  const restart = element("restart-training") as HTMLButtonElement;
  restart.disabled = status.restartPending;
  restart.textContent = status.restartPending ? "Restart accepted" : "Create new run";
  return status;
}

function renderGeneration(result: ToolCallResult): void {
...
function renderAgentActivity(name: string, result: JsonObject): void {
...
function renderAgentFailure(name: string, error: unknown): void {
...
async function refresh(): Promise<void> {
  const button = element("refresh") as HTMLButtonElement;
...
async function readGeneration(): Promise<void> {
  try {
...
async function startTraining(): Promise<void> {
  const button = element("start-training") as HTMLButtonElement;
  button.disabled = true;
  try {
...
async function trainingCommand(name: string, argumentsValue: JsonObject): Promise<void> {
  try {
...
function expectedRevision(): number {
  if (!lastTrainingStatus) throw new Error("training status is unavailable");
  return lastTrainingStatus.revision;
}

async function setLearningRate(): Promise<void> {
  const value = Number((element("learning-rate") as HTMLInputElement).value);
...
function integerInput(id: string): number {
  const value = Number((element(id) as HTMLInputElement).value);
  if (!Number.isSafeInteger(value)) throw new Error(`${id} must be an integer`);
  return value;
}

async function restartTraining(): Promise<void> {
  const button = element("restart-training") as HTMLButtonElement;
  button.disabled = true;
  try {
    const config = {
      ...
      learningRate: Number((element("config-learning-rate") as HTMLInputElement).value),
    };
...
async function registerWebMcp(): Promise<void> {
  ...
  const listed = await mcpRequest<unknown>("tools/list");
  ...
  for (const tool of tools) {
    ...
    await document.modelContext.registerTool({
      ...
      execute: async (argumentsValue, options) =>
        executeWebMcpTool(tool, argumentsValue, options),
    });
  }
