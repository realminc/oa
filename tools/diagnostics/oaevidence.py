...
def _external_file_record(path: pathlib.Path) -> dict[str, Any]:
...
def _explicit_validation_layer_provenance() -> dict[str, Any] | None:
    ...
    for variable in ("VK_LAYER_PATH", "VK_ADD_LAYER_PATH"):
        for raw_path in os.environ.get(variable, "").split(os.pathsep):
            if raw_path:
                search_roots.append(pathlib.Path(raw_path).expanduser().resolve())
    if not search_roots:
        return None
    ...
    if declared.is_absolute():
        ...
        library = next(
            (
                pathlib.Path(raw_path).expanduser() / declared
                for raw_path in os.environ.get("LD_LIBRARY_PATH", "").split(
                    os.pathsep
                )
                if raw_path
                and (pathlib.Path(raw_path).expanduser() / declared).is_file()
            ),
            None,
        )
        if library is None:
            raise RuntimeError(
                f"{manifest}: library_path {library_name!r} is a bare soname; "
    ...
    return {
        "explicit_manifest": _external_file_record(manifest),
        "explicit_library": _external_file_record(library),
    }
...
def _snapshot_vulkan(staging: pathlib.Path, repo: pathlib.Path) -> dict[str, Any]:
    ...
    validation_layer = _validation_layer_record(summary.stdout)
    if validation_layer is not None:
        try:
            provenance = _explicit_validation_layer_provenance()
        except RuntimeError as error:
            validation_layer["explicit_provenance_error"] = str(error)
        else:
            if provenance is not None:
                validation_layer.update(provenance)
    result["validation_layer"] = validation_layer
