...
def _power_metadata() -> dict[str, Any]:
    ...
    if executable:
        ...
        if listed.returncode == 0 and power_profile:
            current_profile = None
            for line in listed.stdout.splitlines():
                header = re.match(r"^\s*(?:\*\s*)?([A-Za-z0-9_-]+):\s*$", line)
                if header:
                    current_profile = header.group(1)
                    continue
                field = re.match(r"^\s+([A-Za-z][A-Za-z0-9]*):\s*(.*?)\s*$", line)
                if field and current_profile == power_profile:
                    profile_details[field.group(1)] = field.group(2)
    return {
        "cpu_governors": sorted(governors),
        "power_profile": power_profile,
        "power_profile_details": profile_details,
    }


def _thermal_snapshot() -> list[dict[str, Any]]:
    ...
    for root in sorted(pathlib.Path("/sys/class/hwmon").glob("hwmon*")):
        device = _read_text(root / "name") or root.name
        try:
            device_identity = (root / "device").resolve(strict=True).name
        except OSError:
            device_identity = root.name
        for input_path in sorted(root.glob("temp*_input")):
            ...
            label = _read_text(root / f"{prefix}_label") or prefix
            sensors.append({
                "sensor": f"{device}:{device_identity}:{label}",
                "celsius": celsius,
            })
    return sensors


def _thermal_summary(samples: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _wait_for_thermal_limit(
    *,
    limit_celsius: float | None,
    sensor_pattern: re.Pattern[str] | None,
    timeout_seconds: float,
) -> tuple[list[dict[str, Any]], float, float | None]:
    started = time.monotonic()
    while True:
        snapshot = _thermal_snapshot()
        matching = [
            float(sensor["celsius"])
            for sensor in snapshot
            if float(sensor["celsius"]) > 0.0
            and (
                sensor_pattern is None
                or sensor_pattern.search(str(sensor["sensor"])) is not None
            )
        ]
        maximum = max(matching) if matching else None
        elapsed = time.monotonic() - started
        if limit_celsius is None:
            return snapshot, elapsed, maximum
        if maximum is None:
            raise ValueError("thermal gate matched no positive-temperature sensor")
        if maximum <= limit_celsius:
            return snapshot, elapsed, maximum
        if elapsed >= timeout_seconds:
            raise TimeoutError(
                f"thermal gate timed out at {maximum:.1f} C "
                f"(limit {limit_celsius:.1f} C)"
            )
        time.sleep(min(5.0, timeout_seconds - elapsed))
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--thermal-limit-celsius", type=float)
    parser.add_argument("--thermal-sensor-regex")
...
def run(args: argparse.Namespace) -> tuple[pathlib.Path, int]:
    ...
    if args.thermal_limit_celsius is not None and args.thermal_limit_celsius <= 0.0:
    ...
    if args.max_spread_percent is not None and args.max_spread_percent <= 0.0:
    ...
    thermal_pattern = (
        re.compile(args.thermal_sensor_regex)
        if args.thermal_sensor_regex else None
    )
    ...
    for index in range(total):
        ...
        thermal_before, thermal_wait_seconds, thermal_start_max = (
            _wait_for_thermal_limit(
                limit_celsius=args.thermal_limit_celsius,
                sensor_pattern=thermal_pattern,
                timeout_seconds=args.thermal_timeout,
            )
        )
        started = time.monotonic()
        result = subprocess.run(
        ...
        sample = {
            ...
            "thermal_start_max_celsius": thermal_start_max,
            "stdout": {
    ...
    if (
        stats is not None
        and args.max_spread_percent is not None
        and float(stats["spread_percent"]) > args.max_spread_percent
    ):
        failure = True
    ...
    document: dict[str, Any] = {
        ...
        "host": {
            ...
            "thermal": _thermal_summary(samples),
        },
