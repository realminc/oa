...
class OaEvidenceTest(unittest.TestCase):
    ...
    @mock.patch.object(oaEvidence, "_findRegistry", return_value=None)
    def testCollectsGraphAndBuildProvenance(self, _registry, _vulkan) -> None:
        ...
        actual, exitCode = oaEvidence.collect(args)
    ...
    @mock.patch.object(oaEvidence, "_findRegistry", return_value=None)
    def testWrapsWorkloadAndObservesValidation(self, _registry, _vulkan) -> None:
        ...
        _, exitCode = oaEvidence.collect(args)
        self.assertEqual(exitCode, 0)
        manifest = json.loads((output / "manifest.json").read_text(encoding="utf-8"))
    ...
    def testExtractsValidationLayerVersion(self) -> None:
        summary = """
VK_LAYER_KHRONOS_validation       Khronos Validation Layer       1.4.354  version 2
VK_LAYER_MESA_device_select       Linux device selection layer   1.4.303  version 1
"""
        self.assertEqual(
            oaEvidence._validationLayerRecord(summary),
            {
    ...
    def testRecordsExactExplicitValidationLayerFiles(self) -> None:
        manifests = self.root / "layer/share/vulkan/explicit_layer.d"
        libraries = self.root / "layer/lib"
        manifests.mkdir(parents=True)
        libraries.mkdir(parents=True)
        manifest = manifests / "VkLayer_khronos_validation.json"
        library = libraries / "libVkLayer_khronos_validation.so"
        manifest.write_text(
            json.dumps(
                {
                    "layer": {
        ...
        library.write_bytes(b"exact-layer-binary")
        with mock.patch.dict(
            "os.environ",
            {
                "VK_LAYER_PATH": str(manifests),
        ...
        self.assertEqual(
            provenance["explicit_manifest"]["path"], str(manifest.resolve())
        )
        self.assertEqual(
            provenance["explicit_library"]["path"], str(library.resolve())
        )
        self.assertEqual(len(provenance["explicit_manifest"]["sha256"]), 64)
        self.assertEqual(len(provenance["explicit_library"]["sha256"]), 64)

    def testRejectsBareExplicitLayerWithoutLibraryPath(self) -> None:
        manifests = self.root / "layer-only"
        manifests.mkdir()
        (manifests / "VkLayer_khronos_validation.json").write_text(
            json.dumps(
                {
                    "layer": {
                        "name": "VK_LAYER_KHRONOS_validation",
        ...
        with mock.patch.dict(
            "os.environ", {"VK_LAYER_PATH": str(manifests)}, clear=True
        ):
            with self.assertRaisesRegex(RuntimeError, "set LD_LIBRARY_PATH"):
                oaEvidence._explicitValidationLayerProvenance()

    def testResolvesManifestRelativeValidationLayerLibrary(self) -> None:
        manifests = self.root / "relative/share/vulkan/explicit_layer.d"
        library = self.root / "relative/lib/libVkLayer_khronos_validation.so"
        manifests.mkdir(parents=True)
        library.parent.mkdir(parents=True)
        library.write_bytes(b"relative-layer-binary")
        (manifests / "VkLayer_khronos_validation.json").write_text(
            json.dumps(
                {
                    "layer": {
    ...
    @mock.patch.object(oaEvidence, "_findRegistry", return_value=None)
    def testRejectsNonOaGraphSchema(self, _registry, _vulkan) -> None:
