...
Crypto schemas under `tools/fnAutogen/schema/`. The live count is derived from
...
python3 tools/fnAutogen/check_schema_coverage.py
python3 tools/fnAutogen/check_public_operation_coverage.py \
...
## Layout
  ...
  - `output/` — checked-in generated review fixtures mirroring live outputs
  - `README.md` — this file
...
`python3 tools/check_autogen_drift.py` additionally compares the checked-in
preview trees and rejects generator-owned files which are no longer emitted. A
clean gate therefore proves that live source and review fixtures match the same
idempotent generation and that dead placeholder artifacts have not survived a
schema change.

`python3 tools/fnAutogen/check_schema_coverage.py` is the schema-row depth gate. It
...
schema row. `check_public_operation_coverage.py` is the independent breadth
...
## Adding a new op
  ...
  4. Run `python3 tools/check_autogen_drift.py`; it must report the intended
...
diff -u source/cpp/lib/oa/core/shader/compute/flat/add.slang \
        tools/fnAutogen/output/cpp/lib/oa/core/shader/compute/flat/add.gen.slang

# Compare the elemwise .cpp
diff -u source/cpp/lib/oa/core/fnmatrix/elemwise/fnMatrixElemwise.cpp \
        tools/fnAutogen/output/cpp/lib/oa/core/fnmatrix/elemwise/fnMatrixElemwise.gen.cpp
```
