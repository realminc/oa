        ...
        for path in _iter_sources(root):
            ...
            with path.open("r", encoding="utf-8", errors="replace") as stream:
                for line_number, line in enumerate(stream, 1):
                    ...
                    if not match:
                        continue
                    include_root = match.group(1)
                    target_module = match.group(2)
                    if include_root != "oa":
                        errors.append(
                            f"non-canonical include root {include_root}: "
                    ...
                    sites.append(
                        IncludeSite(
                            ...
                            line=line_number,
                            include=f"oa/{target_module}/{match.group(3)}",
                        )
                    )
...
def check(repo: Path, config_path: Path, baseline_path: Path) -> CheckResult:
    ...
    public_root = repo / "source/cpp/include/oa"
    ...
    public_text = "\n".join(
        ...
        for path in _iter_sources(public_root)
    )
    for symbol in config.get("forbidden_public_symbols", []):
        if re.search(rf"\b{re.escape(symbol)}\b", public_text):
