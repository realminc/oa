#!/usr/bin/env python3
"""Emit a UsdSkel ASCII USD (.usda) of the walking skeleton.

Same skeleton and clean walk cycle as walkToFbx.py, written as a UsdSkel clip:
...
Reuses the bone layout and motion from walkToFbx.py so FBX and USD stay in sync.

Usage:  python tools/gen/animation/walkToUsda.py --out <path.usda> [--frames 72 --fps 30]
"""
...
def main():
    ...
    jointsTok = ", ".join('"%s"' % paths[n] for n in names)
    bind = ", ".join(mat4Translate(*worldPos(n)) for n in names)
    restm = ", ".join(mat4Translate(*rest[n]) for n in names)
    ...
    for i, t in enumerate(secs):
        ...
        for n in names:
            ...
            trRow.append("(%s, %s, %s)" % tuple(repr(float(v)) for v in lt))
            w, x, y, z = eulerDegToQuat(*(ro if ro is not None else (0.0, 0.0, 0.0)))
            roRow.append("(%s, %s, %s, %s)" % (repr(w), repr(x), repr(y), repr(z)))
        transTs.append("            %d: [%s]," % (i, ", ".join(trRow)))
        rotTs.append("            %d: [%s]," % (i, ", ".join(roRow)))
    ...
    a("    )")
    a("    {")
    a("        uniform token[] joints = [%s]" % jointsTok)
