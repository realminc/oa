#!/usr/bin/env python3
"""Emit a valid ASCII FBX 7500 walking skeleton.
...
Usage:  python tools/gen/animation/walkToFbx.py --out <path.fbx> [--frames 72 --fps 30]
"""
...
def main():
    ...
    ids = IdGen()
    boneId = {n: ids.next() for n, _, _ in BONES}
    attrId = {n: ids.next() for n, _, _ in BONES}

    times = [int(round(i * FBX_TIME_PER_SEC / fps)) for i in range(frames)]
    secs = [i / fps for i in range(frames)]
    stopTime = times[-1] if times else 0

    # Build animation tracks (skipped entirely in --static mode).
    anim = []
    for name, _, _ in ([] if args.static else BONES):
        hasT = any(walkFrame(name, s)[0] is not None for s in secs)
        hasR = any(walkFrame(name, s)[1] is not None for s in secs)
        for kind, prop, sel in (("T", "Lcl Translation", 0), ("R", "Lcl Rotation", 1)):
            if (kind == "T" and not hasT) or (kind == "R" and not hasR):
                continue
            cols = [[], [], []]
            for s in secs:
                v = walkFrame(name, s)[sel] or (0.0, 0.0, 0.0)
                for k in range(3):
                    cols[k].append(v[k])
            anim.append({
    ...
    stackId, layerId, docId = ids.next(), ids.next(), ids.next()
    nNodes = len(anim)
    nCurves = sum(len(a["curves"]) for a in anim)
    ...
    w("\t\tP: \"TimeSpanStop\", \"KTime\", \"Time\", \"\",%d" % stopTime)
    w("\t}")
    w("}")
    ...
    total = 1 + len(BONES) * 2 + 2 + nNodes + nCurves
    w("Definitions:  {")
    w("\tVersion: 100")
    w("\tCount: %d" % total)
    ...
    for name, parent, _ in BONES:
        if parent is None:
            w("\tNodeAttribute: %d, \"NodeAttribute::%s\", \"Root\" {" % (attrId[name], name))
            w("\t\tTypeFlags: \"Null\", \"Skeleton\", \"Root\"")
            w("\t}")
        else:
            w("\tNodeAttribute: %d, \"NodeAttribute::%s\", \"LimbNode\" {" % (attrId[name], name))
            w("\t\tProperties70:  {\n\t\t\tP: \"Size\", \"double\", \"Number\", \"\",6\n\t\t}")
            w("\t\tTypeFlags: \"Skeleton\"")
            w("\t}")
    for name, parent, rest in BONES:
        w("\tModel: %d, \"Model::%s\", \"LimbNode\" {" % (boneId[name], name))
        w("\t\tVersion: 232")
        w("\t\tProperties70:  {")
        ...
        w("\t}")
    if anim:
        w("\tAnimationStack: %d, \"AnimStack::%s\", \"\" {" % (stackId, TAKE))
        w("\t\tProperties70:  {")
        w("\t\t\tP: \"LocalStart\", \"KTime\", \"Time\", \"\",0")
        w("\t\t\tP: \"LocalStop\", \"KTime\", \"Time\", \"\",%d" % stopTime)
        w("\t\t\tP: \"ReferenceStart\", \"KTime\", \"Time\", \"\",0")
        w("\t\t\tP: \"ReferenceStop\", \"KTime\", \"Time\", \"\",%d" % stopTime)
        w("\t\t}")
        w("\t}")
    ...
    w("Connections:  {")
    for name, parent, _ in BONES:
        dst = 0 if parent is None else boneId[parent]
        w("\t;Model::%s, Model::%s" % (name, parent if parent else "RootNode"))
        w("\tC: \"OO\",%d,%d" % (boneId[name], dst))
        w("\t;NodeAttribute::%s, Model::%s" % (name, name))
        w("\tC: \"OO\",%d,%d" % (attrId[name], boneId[name]))
    w("\t;AnimLayer::Base Layer, AnimStack::%s" % TAKE)
    w("\tC: \"OO\",%d,%d" % (layerId, stackId))
    for a in anim:
        w("\tC: \"OO\",%d,%d" % (a["node_id"], layerId))
        w("\tC: \"OP\",%d,%d, \"%s\"" % (a["node_id"], boneId[a["bone"]], a["prop"]))
    ...
    if anim:
        ...
        w("\t\tLocalTime: 0,%d" % stopTime)
        w("\t\tReferenceTime: 0,%d" % stopTime)
        w("\t}")
        w("}")
    ...
    print("  bones=%d  curvenodes=%d  curves=%d  frames=%d @ %g fps (%.2fs)"
          % (len(BONES), nNodes, nCurves, frames, fps, frames / fps))
