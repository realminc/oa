...
Usage:
    python bakeClipText.py --cmp /path/to/CombatMotionProcessed \
...
def main() -> None:
    ...
    ap.add_argument("--batch-size", dest="batchSize", type=int, default=128,
    ...
    try:
        ...
        raise SystemExit(
            "bakeClipText.py requires torch and transformers; install them in "
    ...
    flatCaptions = [caption for _, captions in records for caption in captions]
    allFeatures = np.empty((len(flatCaptions), dim), dtype=np.float32)
    for start in range(0, len(flatCaptions), args.batchSize):
        end = min(start + args.batchSize, len(flatCaptions))
        allFeatures[start:end] = feat(flatCaptions[start:end]).astype(np.float32)
        print(f"  encoded {end}/{len(flatCaptions)} captions", end="\r", flush=True)
    if flatCaptions:
        print()
    ...
    for cid, captions in records:
        ...
        np.save(out / f"{cid}.npy", allFeatures[offset:end])
        offset = end
